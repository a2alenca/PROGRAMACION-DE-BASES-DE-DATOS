...
// CREAR TABLAS Y RELLENADO DE ESTAS  ----------------------------------------------------------
...
DECLARE
    ...
    v_nom_full    USUARIO_CLAVE.nombre_empleado%TYPE;
...
BEGIN
    ...
    -- mes y año
    v_periodo := TO_CHAR(v_hoy, 'MMYYYY');

    --  valores de 100 hasta 320
    FOR r IN (
        SELECT e.*, ec.nombre_estado_civil 
        FROM empleado e 
    ...
    LOOP
        ...
        v_anos := EXTRACT(YEAR FROM v_hoy) - EXTRACT(YEAR FROM r.fecha_contrato);

        -------- CREACION DE USUARIO -----------

        v_usr := LOWER(SUBSTR(r.nombre_estado_civil, 1, 1)) ||  --A. letra estado civil minuscula
                 UPPER(SUBSTR(r.pnombre_emp, 1, 3)) ||          --B.Primeras 3 letras nombre del empleado
                 LENGTH(r.pnombre_emp) || '*' ||                -- C. LARGO nombre empleado y D. ASTERISCO
                 SUBSTR(TO_CHAR(r.sueldo_base), -1) ||          -- E. ULTIMO DIJITO SUELDO BASE
                 r.dvrun_emp ||                                 -- f. Digito run empleado
                 v_anos;                                        -- g. años que lleva en empresa....
        
        -- H. SI LLEVA MENOS DE 1 AÑOS LLEVA X
        -- menos de 10 años con x
        IF v_anos < 10 THEN
            v_usr := v_usr || 'X';
        END IF;
        ...
        -- d. DOS LETRAS DEL apellido paterno deacuerdo a....
            IF r.nombre_estado_civil IN ('CASADO', 'ACUERDO DE UNION CIVIL') THEN
            -- Caso: Casado o Union Civil -> 2 primeras letras
            v_letras_ape := LOWER(SUBSTR(r.appaterno_emp, 1, 2));
            
           ELSIF r.nombre_estado_civil IN ('DIVORCIADO', 'SOLTERO') THEN
            -- Caso: Divorciado o Soltero -> Primera y ultima letra
            v_letras_ape := LOWER(SUBSTR(r.appaterno_emp, 1, 1) || SUBSTR(r.appaterno_emp, -1));
            
            ELSIF r.nombre_estado_civil = 'VIUDO' THEN
            -- Caso: Viudo -> Antepenultima y penultima
            v_letras_ape := LOWER(SUBSTR(r.appaterno_emp, -3, 2));
            
           ELSE
            -- Caso: Separado -> Las 2 ultimas letras
            v_letras_ape := LOWER(SUBSTR(r.appaterno_emp, -2));
            END IF;
            
         --- fin d.
         
        -- C. Sacarlos ultimos 3 digitos del sueldo. menos 1
        v_monto_corta := SUBSTR(TO_CHAR(r.sueldo_base - 1), -3);

        -- Juntar todo para la clave final
        v_pass := SUBSTR(TO_CHAR(r.numrun_emp), 3, 1) ||  -- a.3 digito del rut
                  (EXTRACT(YEAR FROM r.fecha_nac) + 2) || -- b.año nacimiento mas 2
                  v_monto_corta ||                        -- c. 3 digito sueldo menos 1
                  v_letras_ape ||                         -- d. 2 letra apellido con condiciones
                  r.id_emp ||                             -- e. id empleado
                  v_periodo;                              -- f.mes y año base dato

        -- 3. INSERTAR EN LA TABLA
        INSERT INTO USUARIO_CLAVE (id_emp, numrun_emp, dvrun_emp, nombre_empleado, nombre_usuario, clave_usuario)
        VALUES (r.id_emp, r.numrun_emp, r.dvrun_emp, 
                r.pnombre_emp || ' ' || r.snombre_emp || ' ' || r.appaterno_emp || ' ' || r.apmaterno_emp, 
                v_usr, v_pass);
        
        v_contador := v_contador + 1;
    END LOOP;

    -- Confirmar si se insertaron filas
    IF v_contador > 0 THEN
        COMMIT;
        DBMS_OUTPUT.PUT_LINE('FIN - Filas: ' || v_contador);
    END IF;
